...
* #5 let's move on to our IV design - and our first stage
...
esttab ivreg2 using IVRegression.rtf, $tableoptions keep(monthsinjail) 
...
*In the research design above (using randomized judges), the always-takers are the defendants (the accused parties) who are always sentenced for a long time no matter who the judge is. The never-takers are the defendants (the accused parties) who are always sentenced for a short time no matter who the judge is. The compliers are the defendants (the accused parties) who are sentenced for a long time only if they get a republican judge. The defiers are the defendants (the accused parties) who are sentenced for a short time only if they get a republican judge. 
 
 
* #13 Comment on the monotonicity assumption and the possibility of "defiers" in this setting. 
* A monotonicity assumption is also known as the no-defiers assumption. This assumption makes sense in a lot of applications. Here, the possibility of defiers means that a defendant will get a short prison time only if he or she gets a republican judge; which given our prior analysis seems unlikely.


 *# 14 In your dataset, what types of defendants are compliers? 
 *Those defendants whose sentence is increased by having a republican judge.
 
 * # 15 Does the cycle of crime hypothesis appear to be true for the compliers? 
 * Yes it does. In #9 I was able to show that the coefficient of monthsinjail on recidivates is .0442798 and significant for the compliers. For these convicts, the severity of the crime actually has a negative impact on a second sentence (beta = -.615, p <.001). Hence, this is a pretty clear indication of the cycle of crime. The cycle of crime hypothesis seems to be true for those who receive a republican judge. More sepcifically, The Cycle of Crime hypothesis appears to be true for compliers, which here is "defendants whose sentence is increased by having a republican judge." 
